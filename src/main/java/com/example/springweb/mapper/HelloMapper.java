...
public interface HelloMapper {
    ...
    HelloUser getPassword(String id);

    @Select("select * from AppInfo where userId = #{id}")
    @Results({
            ...
            @Result(property = "safeLevel", column = "SaftyLevel"),
    })
    List<App> getallApp(String id);

    @Insert("insert into AppInfo values(#{userId},#{applyId},#{AppName},#{AppType},#{safeLevel},#{CreateDate},#{state})")
    void insertAppInfo(App helloapp);
